...
% \newcommand{\customer}{大和総研殿}
% \newcommand{\customer}{住信情報サービス殿}
%\newcommand{\customer}{キヤノン株式会社殿}
...
必要に応じて、スクリプトのカスタマイズやデバッグ作業を容易に行うために、
PowerShellスクリプトの簡易エディタをインストールします。

\begin{enumerate}
 \item PowerGUIのインストール
\end{enumerate}
...
\begin{enumerate}
 \item {\tt E:/}の下に、{\tt VTILLAB-<version>.zip}の中の{\tt
       VTILLAB-<version>}フォルダを展開。以下{\tt \$VTILROOT}を{\tt
       E:/VTILLAB-<version>}として使用します。
...
\end{verbatim}
 ...
 \item {\tt \$VTILROOT/bin/CONFIG.ps1}を編集。必ず設定が必要になるのは
       以下の3項目。
...
\begin{enumerate}
 \item {\tt \$VTILROOT/bin/dumpperfdata.ps1}を右クリックし、{\tt 編集}を選択
...
\end{verbatim}
 \item 上記スクリプトが正常に終了したのを確認後、同様の手順で \\
       {\tt \$VTILROOT/bin/rollupperfdata.ps1}を実行。

 \item タスクの設定。{\tt \$VTILROOT/bin/vtiljob.bat}を定期的に実行する
       Windowsタスクを作成
\end{enumerate}
...
\begin{enumerate}
 \item {\tt Excel 2007}の場合は{\tt
       \$VTILROOT/report/excel/VTIL\_CapacityAnalysis\_TMPL.xlsm}を、
       {\tt Excel 2003}の場合は{\tt
       \$VTILROOT/report/excel/VTIL\_CapacityAnalysis\_TMPL.xls}を、
       チャート作成PCでオープン
...
キャパシティ分析ブックは各クラスタ毎に作成されます。初回作成後、データを
更新するためには、{\tt CONSOLE-<cluster>}シートの「データの更新」ボタ
ンをクリック。
